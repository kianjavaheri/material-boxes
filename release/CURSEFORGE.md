...
- **Tidy by itself.** Breaking a chest removes its Material Box, and one that disappears any other way is kept as missing until you clear it. Double chests keep each half's contents when split or joined. A broken shulker Material Box is kept as "picked up" (its items still count) and reconnects when you place and open it again.
